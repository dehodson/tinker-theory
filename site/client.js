...
socket.on('turn', function( data ) {
    ...
    window.setTimeout(function(){
        if(data.bool){
            document.getElementById("turn").innerText = "Attacking";
        }else{
            document.getElementById("turn").innerText = "Defending";
        }
        yourTurn = data.bool;
        document.getElementById("turn-counter").innerText = "Turn "+data.turnNumber+"/20";
    }, timeout);
});
